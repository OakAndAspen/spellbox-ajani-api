...
class UtilityService
{
    ...
    /**
     * Returns the shared languages between a set and a user
     * @param Edition $set
     * @param User $user
     * @return array
     */
    public static function getSharedLanguages(Edition $set, User $user)
    {
        $setLanguages = $set->getLanguages();
        $userLanguages = $user->getLanguages();
        $languages = [];

        foreach ($setLanguages as $sl) {
            foreach ($userLanguages as $ul) {
                if ($sl == $ul) array_push($languages, $sl);
            }
        }
        return $languages;
    }
